        ...
        callbacks: {
          label: function(context) {
            if (context.datasetIndex === 0) {
              ...
              const percentage = ((yValue / target) * 100).toFixed(1);
              return `${context.dataset.label}: ${yValue}${nutritionData[0].unit} (${percentage}% of target)`;
            }
            const yValue = context.parsed?.y ?? 0;
            return `${context.dataset.label}: ${yValue}${nutritionData[0].unit}`;
          }
        }
      },
    },
