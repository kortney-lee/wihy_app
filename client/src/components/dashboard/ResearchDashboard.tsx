...
const ResearchDashboard: React.FC<ResearchDashboardProps> = ({
  ...
  const handleSearchSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (searchQuery.trim()) {
      setActiveWorkspace(searchQuery.trim());
    }
  };

  // If workspace is active, show ResearchPanel
  if (activeWorkspace) {
    return (
      <div className="w-full h-full">
        <ResearchPanel
          windowWidth={windowWidth}
          initialQuery={activeWorkspace}
          ...
          <form onSubmit={handleSearchSubmit} className="flex gap-2">
            <div className="flex-1 relative">
              ...
              <input
                ...
                placeholder="Search studies, topics, ingredients, claims…"
          ...
          {showFilters && (
            <div className="mt-3 p-4 bg-gray-50 rounded-lg border border-gray-200">
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 text-sm">
                ...
                <div>
                  <label className="block text-gray-700 font-medium mb-1">Study Type</label>
                  <select className="w-full px-3 py-2 border border-gray-300 rounded-md">
                ...
                <div>
                  <label className="block text-gray-700 font-medium mb-1">Population</label>
                  <select className="w-full px-3 py-2 border border-gray-300 rounded-md">
                ...
                <div>
                  <label className="block text-gray-700 font-medium mb-1">Evidence Level</label>
                  <select className="w-full px-3 py-2 border border-gray-300 rounded-md">
                ...
                <div className="flex flex-wrap gap-2 justify-center">
                  {['Creatine', 'Vitamin D', 'Intermittent fasting', 'Seed oils'].map((topic) => (
                    <button
                      key={topic}
                      type="button"
                      onClick={() => setActiveWorkspace(topic)}
