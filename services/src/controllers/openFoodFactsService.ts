...
class OpenFoodFactsService {
  private readonly baseUrl: string = 'https://world.openfoodfacts.org';
  private readonly userAgent: string = 'vHealth-App/1.0 (https://vhealth.com)';

  /**
   * Controller method: Get product by barcode
   */
  async getProductByBarcodeController(req: Request, res: Response): Promise<void> {
    try {
   ...
   */
  async searchProductsController(req: Request, res: Response): Promise<void> {
    try {
      ...
      const result = await this.searchProducts(query, parseInt(limit as string));
      res.json(result);
    } catch (error) {
   ...
   */
  async getNutritionFactsController(req: Request, res: Response): Promise<void> {
    try {
      const { barcode } = req.params;
   ...
   */
  async getProductCategoriesController(req: Request, res: Response): Promise<void> {
    try {
      const { barcode } = req.params;
   ...
   */
  async checkServiceStatusController(req: Request, res: Response): Promise<void> {
    try {
  ...
  async searchProducts(query: string, limit: number = 10): Promise<ProductSearchResult> {
    try {
      ...
      })).filter((product: any) => product.name !== 'Unknown Product'); // Filter out products without names
   ...
   */
  async getProductCategories(barcode: string): Promise<ProductCategories> {
    try {
      const product = await this.getProductByBarcode(barcode);
      ...
      return {
        success: true,
        categories: rawProduct.categories ? rawProduct.categories.split(',').map((c: string) => c.trim()) : [],
        labels: rawProduct.labels ? rawProduct.labels.split(',').map((l: string) => l.trim()) : [],
        allergens: rawProduct.allergens ? rawProduct.allergens.split(',').map((a: string) => a.trim()) : []
      };
  ...
  private getCalories(product: any): number {
  ...
  private getNutrient(product: any, nutrient: string): number {
    const nutriments = product.nutriments;
  ...
  private createVerdict(product: any): string {
    let verdict = '';
    ...
    if (product.allergens) {
      const allergens = product.allergens.split(',').map((a: string) => a.trim().replace('en:', ''));
...
export const openFoodFactsController = new OpenFoodFactsService();
export default openFoodFactsController;