# Expo + React Native Setup Script for WIHY Health Scanner (Windows PowerShell)
...
Write-Host "🚀 Setting up React Native + Expo for WIHY Health Scanner" -ForegroundColor Green
Write-Host "==================================================" -ForegroundColor Green

# Check if we're in the right directory
if (-not (Test-Path "package.json")) {
    Write-Host "❌ Error: Please run this script from the root of wihy_ui_clean directory" -ForegroundColor Red
    exit 1
}
...
# Create expo app
Write-Host "📱 Creating Expo React Native app..." -ForegroundColor Cyan
...
Write-Host "⚙️ Configuring Expo app..." -ForegroundColor Cyan
...
Write-Host "📁 Creating shared services directory..." -ForegroundColor Cyan
Set-Location ..
...
New-Item -ItemType Directory -Path "shared\constants" -Force | Out-Null

Write-Host "🔄 Copying shared services from web app..." -ForegroundColor Cyan

# Copy API services (these can be reused 100%)
if (Test-Path "client\src\services\wihyAPI.ts") {
    Copy-Item "client\src\services\wihyAPI.ts" "shared\services\" -Force
    Write-Host "✅ Copied wihyAPI.ts" -ForegroundColor Green
}

if (Test-Path "client\src\services\newsService.ts") {
    Copy-Item "client\src\services\newsService.ts" "shared\services\" -Force
    Write-Host "✅ Copied newsService.ts" -ForegroundColor Green
}

# Copy session manager
if (Test-Path "client\src\services\sessionManager.ts") {
    Copy-Item "client\src\services\sessionManager.ts" "shared\services\" -Force
    Write-Host "✅ Copied sessionManager.ts" -ForegroundColor Green
}

# Copy platform detection service
if (Test-Path "client\src\services\shared\platformDetectionService.ts") {
    Copy-Item "client\src\services\shared\platformDetectionService.ts" "shared\services\" -Force
    Write-Host "✅ Copied platformDetectionService.ts" -ForegroundColor Green
}

# Copy types if they exist
if (Test-Path "client\src\types") {
    try {
        Copy-Item "client\src\types\*" "shared\types\" -Recurse -Force -ErrorAction SilentlyContinue
        Write-Host "✅ Copied type definitions" -ForegroundColor Green
    }
    catch {
        Write-Host "⚠️  Could not copy type definitions" -ForegroundColor Yellow
    }
}

Write-Host "📱 Creating React Native app structure..." -ForegroundColor Cyan
Set-Location expo-app
...
$layoutContent | Out-File -FilePath "app\_layout.tsx" -Encoding utf8

# Create main search screen
$indexContent = @'
...
$indexContent | Out-File -FilePath "app\index.tsx" -Encoding utf8
...
$resultsContent | Out-File -FilePath "app\results.tsx" -Encoding utf8
...
$researchContent | Out-File -FilePath "app\research.tsx" -Encoding utf8

Write-Host "🎨 Creating shared services adapter..." -ForegroundColor Cyan

# Create shared services adapter
$adapterContent = @'
...
$adapterContent | Out-File -FilePath "services\sharedServicesAdapter.ts" -Encoding utf8

Set-Location ..

Write-Host "✅ React Native + Expo setup complete!" -ForegroundColor Green
Write-Host ""
Write-Host "🎯 Next Steps:" -ForegroundColor Cyan
Write-Host "1. cd expo-app" -ForegroundColor White
Write-Host "2. npx expo start" -ForegroundColor White
Write-Host "3. Scan QR code with Expo Go app on your device" -ForegroundColor White
Write-Host ""
Write-Host "📱 Development Commands:" -ForegroundColor Cyan
Write-Host "• Expo app: 'cd expo-app; npx expo start'" -ForegroundColor White
Write-Host "• Web app:  'cd client; npm start'" -ForegroundColor White
Write-Host ""
Write-Host "🔧 Integration Tasks:" -ForegroundColor Cyan
Write-Host "1. Connect shared services (WiHy API, Session Manager)" -ForegroundColor White
Write-Host "2. Implement image analysis with camera" -ForegroundColor White
Write-Host "3. Add proper navigation between screens" -ForegroundColor White
Write-Host "4. Style components to match web design" -ForegroundColor White
Write-Host "5. Test on physical devices" -ForegroundColor White
Write-Host ""
Write-Host "📖 See REACT_NATIVE_EXPO_INTEGRATION.md for detailed implementation guide" -ForegroundColor Cyan