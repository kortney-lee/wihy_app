 ...
 * User API Client
 * 
 * HTTP client for user.wihy.ai with JWT authentication.
 * Handles user profiles, preferences, family/coach data.
 * 
...
class UserApiClient {
  ...
  async request<T>(
    ...
    try {
      ...
      console.error(`[UserAPI] ${config.method} ${endpoint} failed:`, error.message);
      throw error;
    }
  }
  ...
  async getWithCache<T>(
    ...
    if (isOnline) {
      try {
        ...
        if (cached) {
          console.warn(`[UserAPI] Using cached data for ${endpoint}`);
          return {
            data: cached.data,
            isStale: true,
            fromCache: true,
